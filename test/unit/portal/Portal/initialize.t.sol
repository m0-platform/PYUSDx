...
contract InitializeUnitTest is PortalUnitTestBase {
    ...
    function test_initialize_zeroAdmin() external {
        bytes memory initializeData = abi.encodeCall(Portal.initialize, (address(0), pauser, operator));
    ...
    function test_initialize_zeroPauser() external {
        bytes memory initializeData = abi.encodeCall(Portal.initialize, (admin, address(0), operator));
    ...
    function test_initialize_zeroOperator() external {
        bytes memory initializeData = abi.encodeCall(Portal.initialize, (admin, pauser, address(0)));
